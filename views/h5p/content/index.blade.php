    ...
    <div class="container-fluid">
        ...
        <div class="row" style="margin-bottom: 10px;">
            ...
            <div class="col-md-9">
                ...
                <form action="{{ route('h5p.index') }}" class="form-inline" method="GET">
                    <fieldset>
                        ...
                        <button type="submit" class="btn btn-primary"><i class="fa fa-search"></i>
                            {{ trans('laravel-h5p.content.search') }}</button>
                    </fieldset>
            ...
            <div class="col-md-3">
                <a href="{{ route("h5p.create") }}"
                    class="btn btn-primary pull-right">{{ trans('laravel-h5p.content.create') }}</a>
            </div>
        </div>
                    ...
                    <tbody>
                        ...
                        @foreach($entrys as $n => $entry)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('h5p.edit', $entry->id) }}" class="btn btn-default"
                                        data-tooltip="{pos:'top'}"
                                        title="{{ trans('laravel-h5p.content.edit') }}">{{ trans('laravel-h5p.content.edit') }}</a>

                                    <button class="btn btn-danger h5p-delete"
                                        data-delete="{{ route('h5p.destroy', $entry->id) }}" data-tooltip="{pos:'top'}"
                                        title="{{ trans('laravel-h5p.content.destroy') }}">{{ trans('laravel-h5p.content.destroy') }}</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
